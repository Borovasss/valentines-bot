...
async def paysupport_command(message: types.Message):
    await message.answer("Для вопросов по оплате пишите: @Borov3223")  # Замените на ваш контакт

@dp.message(Command("support"))
async def support_command(message: types.Message):
    await message.answer("Для общих вопросов пишите: @Borov3223")  # Замените на ваш контакт
    ...
    if message.from_user.id in dp.data:
        ...
        try:
            await moderation_bot.send_message(
                chat_id=5397929249,  # Замените на ваш Telegram ID (например, 123456789)
                text=mod_message
            )
        except Exception as e:
            logging.error(f"Не удалось отправить сообщение в модерационный бот: {e}")
